...
var AuctionContract = new Contract(
  AuctionAddress, AuctionInterface2
)
...
if(fsdfdsfds !== nothing){
  ...
  const { send: stakeTokensSend, state: stakeTokensState } =
    useContractFunction(AuctionContract, "FutureBurn0xEasier", {
      transactionName: "Auction tokens",
    })

  const erc20Interface = new utils.Interface(Erc20.abi)
console.log("Test", AuctionContract)
  ...
  useEffect(() => {
    if (approveErc20State.status === "Success") {
      ...
      stakeTokensSend(era, tests, auctionnumber, Addy, options)
    }
    // the dependency arry
