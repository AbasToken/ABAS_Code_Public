...
import { useEthers, useTokenBalance, useNotifications, useEtherBalance, addressEqual } from "@usedapp/core"
...
import { useStakeTokens, AuctionStats8, getFreeMint, ApproveOnlyMax2, DistributeAuction2, DistributeReward2, AuctionStats7, AuctionStats6, AdvAuctions, AuctionNoApprove, GetApprovalAmt2, AuctionThatCake, StakeThatCake,GetApprovalAmt,  DistributeAuction, AuctionPlayerStats2, useStakeTokens6, useStakeTokens7, AuctionStats, AuctionStats2, useUnstakeTokens4, useUnstakeTokens5, useAuctions1, AuctionStats3, AuctionPlayerStats } from "../../hooks"
...
if(startingauction !== nothing && numberofAuctions > (600- parseFloat(startingauction.toString()) -1)){
  ...
  const formattedTokenBalance: number = tokenBalance
    ...
    var testzz2 = useEtherBalance(account);

  var formattedTokenBalance2: number = testzz2
  ? parseFloat(formatUnits(testzz2, 18))
  : 0 
...
//admin
    const { send: getFreeMintz, state: unstakeTokensState2 } =
    getFreeMint(tokenAddress)
  ...
  const getFreeMintz2 = () => {
    ...
    return getFreeMintz(amountAsWei.toString())
  }
  ...
  const auctionOnly = () => {
    ...
    if(typeof amount === 'number')
    {
      fuck8 =   utils.parseEther(parseFloat(amount.toString()).toFixed(8))
    }
    else{
      fuck8 =  "0"
    }
...
var xxxx = 0
    ...
    if(auctionPot !== nothing)
    {
      ...
      xxx = (parseFloat(xxx2) /  1000000000000000000).toString()
	  
      urpercent = (((parseFloat(amount.toString()) + parseFloat(xxxx.toString())) * 1000000000000000000) / (totalPot + parseFloat(amount.toString()) *  1000000000000000000) * 100)
    
      if(xxxx !== 0){
        urpercentz = ((parseFloat(xxxx.toString()) * 1000000000000000000) / (totalPot ) * 100)
      }
      else{
        urpercentz=0
      }
    ...
    var coinz = (percentzz2/100 * 32768).toFixed(18)
 ...
 var approvetomuch2 = 0 > parseFloat(aprovamt2.toString())
...
if(msgowed-parseFloat(coinz)!== 32768 && msgowed > 0 || networkName === "mumbai"){
  ...
  if(token.address === "0x0000000000000000000000000000000000000003")
{
  return (<>
    <Button
     ...
     >
       {isMining ? <CircularProgress size={26} /> : "Get 0xBTC Test Tokens"}
     </Button>
     <Button
       color="primary"
       variant="contained"
       ...
       disabled={isMining || (isMining3)}
     >
       {isMining ? <CircularProgress size={26} /> : "Get bForge Test Tokens"}
     </Button>
     ...
     <Button
       ...
       variant="contained"
       size="large"
       onClick={getFreeMintz2}
       disabled={isMining || (isMining3)}
     >
       {isMining ? <CircularProgress size={26} /> : "Get ABAS Test Tokens"}
     </Button>
...
if(startingauction !== nothing && parseInt(startingauction?.toString()) !== fsdfosd){
  ...
  var msg6 = "After clicking submit, you will bid " + totz2 + " ETHEREUM Tokens on " +auctionnumber + " auctions.  "+ mx + " in Era #"+ eraz
  var msg7 = ""
...
if(!maxapproved){
  ...
  var day = "Total ETH to bid in Auction #" + currentday+" :"
...
className="BABY"
        ...
        <div> You will receive at most {percentz}% of the current auction or {coinz} ABAS</div>
       <div> {timelftmsg}</div>
       <progress value={secPerDay2-timeleft} max={secPerDay2}></progress>
      
       <div style={{display: 'flex', justifyContent: 'center'}}>{msg7}{tva}</div>
        <h2>Total Size of Current Auction {xxx} Ethereum</h2> 
        <h3> You have bid {xxxx} ETH in the current Auction, you will receive {percentzz}% of the 32,768 Forge Tokens
     <br></br>If no other bids are cast and the auction ends with a dust bid</h3>
        ...
        <div style={{display: 'flex', justifyContent: 'center'}}>
            ...
            <div style={{display: 'flex', justifyContent: 'center'}}>
          <p className="myHeader">Amount - The total Ethereum amount to be bid in all chosen auctions combined<br></br>
         ...
         Ex. Current Auction #1.  I want to bid 1 ETH on each of the next 3 Auctions, not including this auction.<br></br>
          Ex. Starting Auction # = 2, Total # Auctions to bid = 3, Amount = 3 ETH</p></div>
